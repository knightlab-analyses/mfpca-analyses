...
data {
  ...
  int <lower=1> cov_size; // covariance matrix size for marginal distribution of Y
  int <lower=1> cov_starts[N]; // starting index for each subjects' marginal covariance matrix
  int <lower=1> cov_stops[N]; // ending index for each subjects' marginal covariance matrix
...
generated quantities {
  ...
  vector[cov_size] Sigma_y; 
  vector[N] log_lik; // marginal log likelihood
  vector[M] Ynew;

  for (n in 1:N){
    Sigma_y[cov_starts[n]: cov_stops[n]] = to_vector(B[subject_starts[n]: subject_stops[n]] * Theta * cov_alpha * Theta' * 
                                                     B[subject_starts[n]: subject_stops[n]]' + diag_matrix(cov_y_vector[subject_starts[n]: subject_stops[n]]));
    log_lik[n] = multi_normal_lpdf(Y[subject_starts[n]: subject_stops[n]] | zero_M[subject_starts[n]: 
                                        subject_stops[n]], to_matrix(Sigma_y[cov_starts[n]: cov_stops[n]], sum(V[n]), sum(V[n])));
    Ynew[subject_starts[n]: subject_stops[n]] = multi_normal_rng(B[subject_starts[n]: subject_stops[n]] * theta_mu + 
                                                                 B[subject_starts[n]: subject_stops[n]] * Theta * alpha[,n], diag_matrix(cov_y_vector[subject_starts[n]: subject_stops[n]]));                                    
  }
}
