# calculate computation time for 7 modalities based on 1_4_mfpca_t2d_ALL_model_selection.R 
...
chol = df[, c('SubjectUnique', 'CollectionDate', 'CHOLHDL', 'CL4')] 
glu = df[, c('SubjectUnique', 'CollectionDate', 'GLU', 'CL4')] 

# choose 7 measurements
dat_mfpca <- data_mfpca_transformed(df_var=list(microb, metab, protein, cytok, a1c, chol, glu), 
                                    shared_id_unique = 'SubjectUnique', 
									shared_time_var = 'CollectionDate', 
                                    y_var = c('shannon_microb', 'shannon_metab', 'shannon_protein', 
                                              'shannon_cytok', 'A1C', 'CHOLHDL', 'GLU'), 
...
# 2. model selection
#--------------------------
mdl_list <- list(list(num_PCs=c(2,2,2,2,2,2,2), nknots=c(1,1,1,1,1,1,1)))
...
save(diff_time, mdl_list, dat_mfpca,
     file=paste0(dir, '/results/mfpca_t2d_compute_time_mod7.RData'))
