...
dat_mfpca <- data_mfpca_transformed(df_var=list(microb, protein, cytok, a1c), 
                                    ...
                                    y_var = c('shannon_microb', 'shannon_protein', 
                                              'shannon_cytok', 'A1C'), 
...
# 2. model selection
#--------------------------
mdl_list <- list(list(num_PCs=c(2,4,3,4), nknots=c(1,1,1,1)),
                 list(num_PCs=c(2,4,4,4), nknots=c(1,1,1,1)),
                 list(num_PCs=c(2,4,2,4), nknots=c(1,1,1,1)))
...
save(fit, mdl_list, Nsamples, Nchains, pca_data, param_stan, dat_mfpca, 
	 file=paste0(dir, '/results/mfpca_t2d_microb_protein_cytok_a1c_', i, '.RData'))
